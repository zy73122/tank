var MapLayer = cc.LayerColor.extend({
    ...
    preLoadMap: function() {
        ...
        //创建TileMap地图
        var map = this.map = new cc.TMXTiledMap(res.map1_tmx);
...
//      map.setAnchorPoint(0.5, 0.5);
        ...
        //改用图片层来表示对象，取消对象层  （还不行）
        var group = map.getLayer("objects");
        ...
        for (var k in tiles) {
            ...
            if (gid) {
                ...
                if (properties) {
                    var fix_x = 32 / 2;
                    var fix_y = 32 / 2;
                    var objtype = properties.objtype;
                    switch (objtype) {
                    case 'plant1':
                        objs_plant.push({
                            x : object.x + fix_x,
                            y : object.y + fix_y,
                            index : objtype.substring(objtype.indexOf("plant") + 5)
                        });
                        break;
                    case 'enemy1':
                        break;
    ...
    tileCoordForPosition: function(position)
    {
        ...
        var positionInMap = this.convertToNodeSpace(position);
    ...
    rectForTileCoord: function(tilepos)
    {
        ...
        pos = this.convertToWorldSpace(pos); //作用等同于 //x += Game.w/6;
